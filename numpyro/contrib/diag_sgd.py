...
def _compute_cdf_grid(d, M: int):
    ...
    if isinstance(d, (CategoricalProbs, CategoricalLogits)):
        ...
        return jnp.cumsum(jnp.asarray(d.probs), axis=-1)[..., :-1]  # (*B, K−1)

    if isinstance(d, (BinomialProbs, BinomialLogits)):
        # F(k) = I_{1−p}(n−k, k+1)  (regularised incomplete beta)
        p = jnp.asarray(d.probs)[..., None]  # (*B, 1)
...
class SmoothICDFTransform(Transform):
    ...
    This is the raw grid inverse-CDF of Wagner et al.  It is kept as a
    primitive (and for its closed-form inverse), but it is **not** what
    :func:`SmoothedDiscrete` uses: its pushforward density
    :math:`-\\log |\\partial Q_\\eta / \\partial u|` carries no information about
    the pmf -- for a Bernoulli it is :math:`\\eta / (z(1-z))` on the image,
    independent of :math:`p` -- so it is not a usable DSGD density term.  See
    :class:`SmoothedFinite` for the sample and density actually used for
    finite-support families.
...
# ---------------------------------------------------------------------------
# Normalised index-space relaxation for finite-support families
# ---------------------------------------------------------------------------


def relaxed_finite_index(base_dist, u, eta, max_support: Optional[int] = None):
    r"""Normalised index-space relaxation :math:`Q_\eta(u)` of a finite-support
    discrete distribution on ``{low, ..., low + K - 1}``, for :math:`u \in (0, 1)`.

    With :math:`a_k = \sigma_\eta(u - F(k))` at the interior boundaries
    :math:`k = 0, \dots, K-2`, the end caps :math:`a_{-1} = \sigma_\eta(u)`
    (:math:`F(-1) = 0`) and :math:`a_{K-1} = \sigma_\eta(u - 1)`
    (:math:`F(K-1) = 1`), and soft one-hot weights :math:`w_k = a_{k-1} - a_k`,

        :math:`Q_\eta(u) = \mathrm{low} + \frac{\sum_{k=0}^{K-1} k\, w_k}
        {\sum_{k=0}^{K-1} w_k}
        = \mathrm{low} + \frac{\sum_{k=0}^{K-2} a_k - (K-1)\, a_{K-1}}
        {a_{-1} - a_{K-1}}`,

    a convex combination of the support points -- the finite-support counterpart
    of :func:`anchored_relaxed_count`, and the reparameterised sample of
    :class:`SmoothedFinite`.  It differs from the plain sigmoid sum
    :func:`smooth_icdf` (:math:`\mathrm{low} + \sum_k a_k`) only through the
    end caps, which remove the sigmoid tails leaking past :math:`u = 0` and
    :math:`u = 1`.  The relaxed value therefore stays inside
    ``[low, low + K - 1]``, its mean bias vanishes as :math:`\eta \to 0` and
    also as :math:`\eta \to \infty` (where :math:`z \to` the mean pointwise),
    and it is small in between: a Bernoulli with :math:`p = 10^{-3}` at
    :math:`\eta = 0.1` has relaxed mean :math:`\approx 7 \times 10^{-4}`.  It
    is not exactly mean-faithful at finite :math:`\eta`.

    :param base_dist: a finite-support discrete Distribution (Bernoulli,
        Categorical, Binomial, DiscreteUniform).
    :param u: uniform variate(s) in ``(0, 1)``; the return has the same shape.
    :param float eta: smoothing temperature :math:`\eta > 0`.
    :param max_support: optional grid size override.
    """
    M = _resolve_max_support(base_dist, max_support)
    cdf_grid = _compute_cdf_grid(base_dist, M)  # (*batch, M-1)
    low = _support_lower_bound(base_dist)
    a = smooth_heaviside(u[..., None] - cdf_grid, eta)  # a_0 .. a_{M-2}
    a_minus_one = smooth_heaviside(u, eta)  # F(-1) = 0
    a_top = smooth_heaviside(u - 1.0, eta)  # F(M-1) = 1
    numerator = jnp.sum(a, axis=-1) - (M - 1) * a_top
    return low + numerator / (a_minus_one - a_top)


def _finite_log_pmf(base_dist, value):
    r"""Analytic continuation of a finite-support family's log-pmf to real
    ``value`` in ``[low, low + K - 1]``.

    Bernoulli, Binomial and DiscreteUniform already write ``log_prob`` as a
    continuation in the value (``xlogy`` / ``gammaln``, or a constant), so they
    defer to it with support validation bypassed, exactly as the count families
    do.  A Categorical has no arithmetic structure to continue and its
    ``log_prob`` is a table lookup, so its log-pmf is interpolated linearly
    between the two neighbouring categories: with :math:`k = \lfloor z \rfloor`
    and :math:`f = z - k`,

        :math:`\log \tilde p(z) = (1 - f) \log p_k + f \log p_{k+1}`,

    which is the soft one-hot expectation :math:`\sum_k w_k \log p_k` of the
    relaxation whenever only two adjacent categories carry weight (the
    non-overlapping-sigmoid regime), is exact on the integers, and is bounded
    wherever the pmf is.
    """
    if isinstance(base_dist, (CategoricalProbs, CategoricalLogits)):
        log_pmf = jax.nn.log_softmax(base_dist.logits, axis=-1)
        K = log_pmf.shape[-1]
        batch_shape = jnp.broadcast_shapes(jnp.shape(value), base_dist.batch_shape)
        if K == 1:
            return jnp.zeros(batch_shape, dtype=log_pmf.dtype)
        log_pmf = jnp.broadcast_to(log_pmf, batch_shape + (K,))
        lower = jnp.clip(jnp.floor(value), 0, K - 2)
        frac = value - lower
        idx = jnp.broadcast_to(lower.astype(int), batch_shape)[..., None]
        lp_lower = jnp.take_along_axis(log_pmf, idx, axis=-1)[..., 0]
        lp_upper = jnp.take_along_axis(log_pmf, idx + 1, axis=-1)[..., 0]
        return (1.0 - frac) * lp_lower + frac * lp_upper
    if isinstance(
        base_dist,
        (
            BernoulliProbs,
    ...
    raise ValueError(
        f"numpyro.contrib.diag_sgd: no log-pmf continuation for "
        f"{type(base_dist).__name__}."
    )


def _invert_monotone(f, target, n_bisect: int = 40, min_slope: float = 1e-3):
    """Solve ``f(u) = target`` for ``u`` in ``(0, 1)``, ``f`` a monotone
    increasing elementwise map.

    Bisection (under ``stop_gradient``) locates the root robustly even where
    the map has flat plateaus, which defeat Newton from a generic start.  One
    implicit-function step then attaches the gradient: with ``u*`` the detached
    root and ``f'`` its detached slope, ``u = u* - (f(u*) - target) / f'`` has
    value ``u*`` (to rounding) and derivatives ``du/dtarget = 1/f'`` and
    ``du/dtheta = -(df/dtheta)/f'``, with no second-order term -- unlike
    differentiating through a Newton iteration, whose ``(f - target) f''/f'^2``
    term overflows on plateaus where ``f'`` underflows.  Where the slope is
    below ``min_slope`` the root is ill-conditioned in ``u`` and the step is
    skipped: there the soft one-hot weights are essentially constant in ``u``,
    so the missing gradient contribution is O(``f'``).
    """
    target_sg = jax.lax.stop_gradient(target)

    def bisect(carry, _):
        lo, hi = carry
        mid = 0.5 * (lo + hi)
        below = jax.lax.stop_gradient(f(mid)) < target_sg
        return (jnp.where(below, mid, lo), jnp.where(below, hi, mid)), None

    (lo, hi), _ = jax.lax.scan(
        bisect, (jnp.zeros_like(target), jnp.ones_like(target)), None, length=n_bisect
    )
    u_root = jax.lax.stop_gradient(0.5 * (lo + hi))
    value, slope = jax.jvp(f, (u_root,), (jnp.ones_like(u_root),))
    slope = jax.lax.stop_gradient(slope)
    steep = slope > min_slope
    safe_slope = jnp.where(steep, slope, 1.0)
    return jnp.where(steep, u_root - (value - target) / safe_slope, u_root)


def _finite_soft_weights(base_dist, u, eta, max_support):
    r"""Soft one-hot weights :math:`w_k / \sum_j w_j` of
    :func:`relaxed_finite_index` at base uniform ``u``, shape ``(..., K)``:
    :math:`w_k = a_{k-1} - a_k` with the end caps :math:`a_{-1} = \sigma_\eta(u)`
    and :math:`a_{K-1} = \sigma_\eta(u - 1)`.  They satisfy
    :math:`\sum_k k\, w_k / \sum_j w_j = Q_\eta(u) - \mathrm{low}`.
    """
    M = _resolve_max_support(base_dist, max_support)
    cdf_grid = _compute_cdf_grid(base_dist, M)
    a = smooth_heaviside(u[..., None] - cdf_grid, eta)  # (..., M-1)
    lower = smooth_heaviside(u, eta)[..., None]
    upper = smooth_heaviside(u - 1.0, eta)[..., None]
    a_ext = jnp.concatenate([lower, a, upper], axis=-1)  # (..., M+1)
    w = a_ext[..., :-1] - a_ext[..., 1:]  # (..., M)
    return w / jnp.sum(w, axis=-1, keepdims=True)


def _categorical_smoothed_log_pmf(base_dist, value, eta, max_support):
    r"""Exact DSGD smoothing of a Categorical's log-pmf program at relaxed
    ``value``: :math:`\sum_k w_k(u)\, \log p_k`, where ``u`` is recovered from
    ``value`` by inverting the (monotone) relaxed sample map.

    :func:`_finite_log_pmf` interpolates :math:`\log p_k` linearly between the
    two neighbouring categories, which equals this whenever only adjacent
    categories carry weight; the two differ by O(:math:`\eta`) where several
    sigmoids overlap (about 0.1 nats at :math:`\eta = 0.1` for a three-class
    example, below 1e-3 by :math:`\eta = 0.03`).
    """
    log_pmf = jax.nn.log_softmax(base_dist.logits, axis=-1)
    batch_shape = jnp.broadcast_shapes(jnp.shape(value), base_dist.batch_shape)
    value = jnp.broadcast_to(value, batch_shape)
    u = _invert_monotone(
        lambda uu: relaxed_finite_index(base_dist, uu, eta, max_support), value
    )
    w = _finite_soft_weights(base_dist, u, eta, max_support)
    return jnp.sum(w * log_pmf, axis=-1)
...
#
# We instead use a normalised *index-space* relaxation with a runtime-adaptive
# horizon (no truncation).  This construction is this module's own, not one
# taken from Wagner et al.: the DSGD papers smooth branch predicates with plain
# sigma_eta blends (nested blends for nested branches) and never normalise by a
# sum of sigmoid weights.  Writing a_k = sigma_eta(u - F(k)),
...
def adaptive_relaxed_count(
    ...
    a convex combination of the integer outcomes -- a smoothed reparameterised
    sample whose mean bias vanishes as :math:`\eta \to 0`.  The
...
class SmoothedCount(dist.Distribution):
    ...
    whose mean bias vanishes as :math:`\eta \to 0`.  The *density* contribution to the
    ...
    relaxed count satisfies up to a mean bias that vanishes with
    :math:`\eta`.  As :math:`\eta \to 0` the relaxed
...
# ---------------------------------------------------------------------------
# SmoothedFinite: DSGD relaxation of a finite-support discrete distribution
# ---------------------------------------------------------------------------


class SmoothedFinite(dist.Distribution):
    r"""DSGD relaxation of a *finite-support* discrete distribution (Bernoulli,
    Categorical, Binomial, DiscreteUniform).

    The reparameterised sample is the normalised index-space relaxation
    :func:`relaxed_finite_index`, :math:`z_\eta = \sum_k k\, w_k / \sum_k w_k`
    with soft one-hot weights :math:`w_k` built from the sigmoids
    :math:`\sigma_\eta(u - F(k))` of the CDF grid.  The density term is the
    **DSGD smoothing of the log-pmf program** with the same weights,
    :math:`\sum_k w_k \log p_k / \sum_k w_k`, which is the analytic
    continuation of the discrete log-pmf evaluated at :math:`z_\eta`: for a
    Bernoulli the branch ``if u < 1 - p then log(1 - p) else log p`` smooths to

        :math:`\log \tilde p(z_\eta) = z_\eta \log p + (1 - z_\eta) \log (1 - p)`,

    so the temperature enters through :math:`z_\eta` alone and the term
    converges pointwise to the exact log-pmf as :math:`\eta \to 0`.  For a
    Binomial the density is the ``gammaln`` continuation of the binomial
    coefficient plus the same exponential-family term (the exponential-family
    part is exactly the smoothed program; the coefficient part differs from
    :math:`\sum_k w_k \log \binom{n}{k}` by a Jensen gap that vanishes in
    measure as :math:`\eta \to 0`), for a Categorical the exact smoothed
    program :math:`\sum_k w_k \log p_k` with the soft one-hot weights recovered
    by inverting the sample map (:func:`_categorical_smoothed_log_pmf`), and
    for a DiscreteUniform the constant :math:`-\log K`.

    Why not the change-of-variables pushforward density :math:`-\log|dz/du|`
    of the grid :class:`SmoothICDFTransform`: it carries no information about
    the pmf.  For a Bernoulli the transform is the single sigmoid
    :math:`z = \sigma_\eta(u - (1 - p))`, whose pushforward density on the
    image is :math:`\eta / (z(1 - z))` -- independent of :math:`p`.  So
    :math:`\log p(z) - \log q(z)` between two relaxed Bernoullis is identically
    zero where their images overlap (and :math:`\pm\infty` outside) however
    large the exact discrete KL, a DSGD ELBO exerts no prior pressure on any
    Bernoulli site, and a MAP objective sees infinite, parameter-independent
    attractors at both edges of the image.  For :math:`K > 2` the density does
    depend on the probabilities, but at a support point it scales like
    :math:`p_k / (2\eta)` rather than :math:`\log p_k`, so it diverges as
    :math:`\eta \to 0` and its :math:`\log p - \log q` is not the discrete KL
    either.

    The continuation is bounded wherever the pmf is, exact on the integers, and
    affine in :math:`z` for the exponential-family part, so
    :math:`\mathbb{E}_q[\log \tilde p - \log \tilde q]` is a genuine
    :math:`-\mathrm{KL}` surrogate -- exact whenever the soft one-hot weights
    of the relaxed sample have the right expectations,
    :math:`\mathbb{E}_q[w_k] = q_k` (for a Bernoulli, :math:`\mathbb{E}_q[z] =
    q`), which holds up to a mean bias that vanishes as :math:`\eta \to 0` --
    and the smoothed objective converges to the discrete one as
    :math:`\eta \to 0` (Theorem 5.6 of Wagner et al.).

    :param base_dist: a finite-support discrete Distribution.
    :param float eta: smoothing temperature :math:`\eta > 0`.
    :param max_support: optional grid size override.
    """
    ...
    arg_constraints = {}
    pytree_data_fields = ("base_dist", "eta")
    pytree_aux_fields = ("max_support",)
    has_rsample = True

    def __init__(self, base_dist, eta, max_support=None, *, validate_args=None):
        # Fails early for unsupported families rather than at the first sample.
        _resolve_max_support(base_dist, max_support)
        self.base_dist = base_dist
        self.eta = eta
    ...
    def support(self):
        low = _support_lower_bound(self.base_dist)
        high = low + _resolve_max_support(self.base_dist, self.max_support) - 1
        return constraints.interval(low, high)

    def rsample(self, key, sample_shape=()):
        u = jax.random.uniform(key, shape=sample_shape + self.batch_shape)
        return relaxed_finite_index(self.base_dist, u, self.eta, self.max_support)

    def sample(self, key, sample_shape=()):
        return self.rsample(key, sample_shape)

    def log_prob(self, value):
        if isinstance(self.base_dist, (CategoricalProbs, CategoricalLogits)):
            return _categorical_smoothed_log_pmf(
                self.base_dist, value, self.eta, self.max_support
            )
        return _finite_log_pmf(self.base_dist, value)
...
def SmoothedDiscrete(
    ...
    *,
    ...
    relaxation and log-probs are the analytic continuation of the discrete
    log-pmf at the relaxed value (the DSGD smoothing of the log-pmf program).

    Both branches share one construction (relaxed sample + continued log-pmf)
    and differ only in how the relaxed sample is formed:

    * **Finite-support families** (Bernoulli, Categorical, Binomial,
      DiscreteUniform) become a :class:`SmoothedFinite`: the normalised
      index-space relaxation :func:`relaxed_finite_index` over the CDF grid,
      with the continued log-pmf (``z log p + (1 - z) log(1 - p)`` for a
      Bernoulli) as the density.
    * **Unbounded families** (Poisson, Geometric, GammaPoisson /
    ...
    Neither branch uses the change-of-variables pushforward density
    :math:`-\\log|dz/du|` of the grid :class:`SmoothICDFTransform`.  For
    finite support it carries no information about the pmf (for a Bernoulli it
    is independent of ``p``, so ``log p - log q`` vanishes identically and the
    ELBO gets no prior pressure; see :class:`SmoothedFinite`); for unbounded
    support it is unbounded above for near-deterministic distributions and makes
    ``log p - log q`` diverge (see :class:`SmoothedCount`).  With the continued
    log-pmf, ``log p - log q`` is a bounded ``-KL`` surrogate that is exact when
    the relaxed sample's soft one-hot weights have the right expectations and
    otherwise converges to the discrete KL as ``eta -> 0``.
...
class StraightThroughSmoothed(dist.Distribution):
    ...
    ``sample`` draws a smooth reparameterised value ``z`` (the anchored
    index-space relaxation for unbounded families, else the normalised finite
    relaxation :func:`relaxed_finite_index`) and
    ...
    def sample(self, key, sample_shape=()):
        ...
        if _use_count_relaxation(self.base_dist, self.max_support):
            ...
            z = relaxed_finite_index(self.base_dist, u, self.eta, self.max_support)
...
class DSGDMessenger(Messenger):
    ...
    distribution is replaced with :func:`SmoothedDiscrete`, so the sample is
    the smoothed inverse-CDF program and the log-prob is the smoothed log-pmf
    program (the analytic continuation of the discrete log-pmf at the sample).
    This gives the DSGD gradient estimator.
    ...
    relaxation for unbounded families, else :func:`relaxed_finite_index`),
