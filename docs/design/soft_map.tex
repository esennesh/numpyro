...
Table~\ref{tab:failure_rates} reports damped parallel max-product against exhaustive enumeration on random binary pairwise models, over graphs of increasing cyclomatic number.
...
\begin{description}
    ...
    \item[Recovery by adjoint.] Argmaxes and tempered marginals are read off the backward pass. Differentiating a maximum returns an indicator of its argmax~\citep{Danskin1967}, and the backward pass of a contraction \emph{is} the corresponding message-passing algorithm~\citep{Darwiche2003, Eisner2016}; at $T = 0$ it is precisely a Viterbi backtrace.
...
\begin{itemize}
    \item \textbf{Fuse parallel factors into a chain}~\citep{Sennesh2023}. This is a preprocessing step, not an aside: it is what determines the $w$ on which A1's cost depends, and it is available for any straight-line program.
...
Three gaps are worth stating explicitly. First, the equivalence between LP tightness and max-product correctness is established for weighted matching~\citep{Sanghavi2007} but not in the generality in which Section~\ref{sec:minsum} invokes the pattern. Second, countable support at $T = 0$ (Section~\ref{sec:countable}) has no exact method: per-site log-concavity is free in an exponential family but is not closed under max-marginalization, so the method rests either on convolution-structured couplings --- where closure does hold --- or on a truncation bound. Characterizing the largest class of exponential-family couplings whose messages stay discretely convex is the most concrete open problem this document raises, and discrete convex analysis~\citep{Murota2003} is where the answer should be looked for. Third, no analysis here bounds the \emph{number of sweeps} to convergence for A2 or A3; the claim that joint updates need fewer sweeps than coordinatewise ones is argued structurally and observed empirically, not proved.
