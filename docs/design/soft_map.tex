...
\textbf{Countable support below $T = 1$} (Section~\ref{sec:countable}) has no exact method, and is the gap that gates the choice among A1--A4. The generating-function route holds at $T = 1$ alone, so the whole annealing path is outside it; per-site log-concavity is free in an exponential family but is not closed under max-marginalization; and what remains is either convolution-structured couplings, where closure does hold, or a truncation bound. Stated representationally, the target is a finitely parameterized family of functions on $\mathbb{Z}$ closed under pointwise addition and under partial maximization against the model's couplings. The closure theory of M-convex functions is the obvious candidate, but Section~\ref{sec:countable} reports it failing in both directions: our factors are not M$^{\natural}$-concave, and neither is the convolution-structured case whose closure we can prove, so the condition is sufficient and not necessary and cannot be the characterization sought. What the right condition is remains open, and it is the gap that gates the choice among A1--A4.
