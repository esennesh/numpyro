...
\begin{description}
    ...
    \item[Elitism.] The incumbent best assignment is always retained in each site's candidate set, which makes the incumbent objective nondecreasing at fixed $\theta$ (Appendix~\ref{sec:proofs}; Section~\ref{sec:learning} qualifies this once $\theta$ is being learned).
\end{description}
...
\subsection{Learning \texorpdfstring{$\theta$}{theta} argues for particle contraction}
\label{sec:learning}

Once $\theta$ is estimated rather than given, three requirements introduced separately turn out to want the same data structure --- weighted per-site particle sets. The sampling ingredient of Section~\ref{sec:countable} produces per-site candidate sets. The lower bound~\eqref{eq:particle_bound} that makes learning possible is a sum over particles. And correcting for the fact that $\gamma_\theta$ \emph{moves} as $\theta$ is updated is done by carrying importance weights on particles, which is the Jarzynski construction: reweight each particle by the incremental log-ratio of the new target to the old, exactly as in sequential Monte Carlo, so that gradients stay correct under a drifting target rather than relying on the uncontrolled approximation of contrastive divergence~\citep{Carbone2023}. Three independent needs converging on one representation is the strongest argument in this document for taking some form of particle contraction as the spine, and it is an architectural argument rather than a claim about speed.

\paragraph{The two loops use different semirings.} Importance weights live in sum-product; there is no meaningful way to weight a maximum. At $T \rightarrow 0$ weights are irrelevant and one simply retains the best candidate. So Jarzynski reweighting corrects the \emph{learning} loop while elitism drives the \emph{mode-finding} loop, and the nonstationarity correction is not needed for the MAP readout at all --- a mode-finder does not care that $\theta$ moved, it re-optimizes.

\paragraph{Monotonicity becomes per-step.} Proposition~\ref{prop:elitism} holds at fixed $\gamma_\theta$. When $\theta$ moves the incumbent's objective value moves with it, so the guarantee survives only \emph{within} each $\theta$-step: re-evaluate the incumbent under the new $\theta$ --- one density evaluation --- and claim monotonicity from there. Nothing in this document licenses a monotone incumbent across a learning trajectory.

\paragraph{What the reweighting actually protects against.} Bias under a drifting target is the stated motivation, but the sharper one is a failure mode of the coupled loops. Retaining an incumbent across $\theta$-updates while simultaneously fitting $\theta$ to make retained particles likely is self-reinforcing: $\theta$ moves to explain the incumbent, the incumbent stays best, and exploration stops. Keeping a \emph{spread} of weighted particles rather than a single incumbent is what prevents the learning loop from consuming the mode-finder, which is a better reason to carry weights than the bias correction alone.

\paragraph{Two weightings, not one.} The deterministic bound~\eqref{eq:particle_bound} is attained at $w_p \propto \targetm{\theta}{\mathbf{z}_p}$ over deduplicated configurations; the Jarzynski weights are accumulated-work weights giving an unbiased normalizer estimate. Same particles, different weights, different purposes --- substituting one for the other forfeits the bound. And the reweighting carries the usual sequential Monte Carlo cost: as $\theta$ drifts the weights concentrate, so effective sample size needs monitoring and resampling, which reintroduces one level down the same diversity question the weights were introduced to solve.
...
\end{itemize}
Separating them exposes a real seam, and it is wider than the $T = 0$ endpoint. Marginalizing at temperature $T$ evaluates $T \log \sum_n \exp(f_n / T)$, which is the generating function not of the model's own conditionals but of the \emph{tempered} measure $p^{1/T}$ --- and tempering takes a count distribution out of its family. For a Poisson, $p_n^{1/T} \propto \lambda^{n/T} / (n!)^{1/T}$, and $(n!)^{1/T}$ is a factorial only at $T = 1$. The normalizer sometimes remains a recognisable special function --- at $T = \nicefrac{1}{2}$ it is a modified Bessel function, and at $T = \nicefrac{1}{m}$ a generalized hypergeometric --- but there is no closure, and the compositional identities the method relies on do not survive.

The generating-function route is therefore available at exactly \emph{one} point of the homotopy, $T = 1$, rather than at one end of it. In particular a countable-site MAP estimate cannot be reached by annealing down from the exact sum-product end: the representation fails immediately at $T < 1$, not in the limit. Two options remain.
...
\paragraph{Where it does survive.} If the factor couples parent and child by an increment, $z = z' + \varepsilon$, the message is a $(\max,+)$ convolution, and $(\max,+)$ convolution of concave sequences is concave. The proof is one line: a concave sequence is determined by its nonincreasing sequence of first differences, and the convolution's differences are the sorted merge of the two operands' differences, hence again nonincreasing (Appendix~\ref{sec:proofs}). This is exactly the branching, immigration and thinning structure of the integer population models that motivate countable latent variables in the first place~\citep{Sheldon2018}, so it is a natural class rather than a contrived one.

\paragraph{A negative result on discrete convex analysis.} The natural place to look for a general closure theorem is discrete convex analysis, whose M-convex functions are closed under precisely the operations at issue --- infimal convolution and partial minimization --- via the Steinitz exchange property~\citep{Murota1996, Murota2003}. Whether a given factor lies in that class is decidable directly: the M$^{\natural}$-concave exchange property can be checked by enumeration on a finite box, restricted to those point pairs whose required neighbours remain inside the domain so that no violation is an artefact of the boundary.

Carried out, it is negative in both directions. None of the Poisson-chain factors with natural parameter affine in the parent satisfy the exchange property, at any sign or magnitude of the coupling; violations run from 7\% to 29\% of the checked pairs. More informative is that the convolution-structured factor of Appendix~\ref{sec:proofs} --- the one case where $(\max,+)$ closure provably \emph{does} hold --- also fails it, on 29\% of pairs. Separable concave, linear, and concave-in-the-sum functions pass, so the test is not vacuous.

M$^{\natural}$-concavity is therefore sufficient for the closure this section needs but not necessary, and the only structure known to supply that closure lies outside the class. One reason the two do not line up: on $\mathbb{Z}^1$ the M$^{\natural}$ condition degenerates to ordinary discrete concavity, so for univariate messages it carries no content, and its force would have to come from the joint factor on $\mathbb{Z}^{1 + \lvert \parents{z} \rvert}$ --- where the measurement above finds it absent. Proposition~\ref{prop:convolution}'s closure instead comes from one-dimensional concavity under a merge of difference sequences, which is a different mechanism and not an instance of the exchange property.

We record this as a negative result about one candidate route and draw no conclusion here about how countable sites should be represented; the reproduction is in the accompanying script.
...
\paragraph{A third option: sample the neighbourhood.} Both options above try to \emph{represent} a message over an infinite domain, exactly or by cutting the domain down. A candidate set need not do either. Populating each countable site's candidate set by sampling a neighbourhood of its current candidates --- locally-informed proposals of the kind revived for combinatorial optimization by~\citet{Sun2023a}, or with heavy-tailed noise giving polynomial rather than exponential escape from local optima~\citep{Wang2025} --- sidesteps the closure question rather than answering it, because a sampled candidate set is finite by construction and nothing infinite is ever represented. Unlike a truncation it draws no box: a proposal may offer $z_i \pm k$ with probability decaying in $k$, so the support remains $\mathbb{N}$.

This composes with the refinement layer already described rather than competing with it. The proposals in this family are gradient-informed --- they need $\nabla$ of a smooth extension of $\log \targetm{\theta}{\cdot}$ evaluated at integer points --- which is exactly the analytic continuation of Section~\ref{sec:countable}'s second bullet. The smoothing is the ingredient this option consumes.

Three things should be said honestly about it. First, the requirement is weaker than ergodicity: under elitism we do not need the chain to mix with respect to the target, only to \emph{find improvements}, and a proposal that mixes badly while ascending well is adequate. This matters because guaranteed mixing would demand logarithmic cooling~\citep{Geman1984}, which is not a practical budget. Second, hyperparameters --- neighbourhood radius, path length, tail index --- are the known weak point of this literature, and by the standard Table~\ref{tab:updates} applies elsewhere (tuning cost is what separates its \cmark\ from its \xmark) that cost must be scored rather than waived. Third, and least obvious: \textbf{elitism buys a monotone incumbent, not a convergent fixed point.} If candidate sets are resampled between sweeps then messages are computed over a shifting domain, so A2 and A4 may never settle, and message residual stops being a meaningful diagnostic because the domain moved beneath it. A1 is unaffected, being a single contraction. For the iterative algorithms the implication is a design constraint: freeze candidate sets within an outer round and resample only between rounds, or track convergence on the incumbent objective rather than on messages. Note also that Proposition~\ref{prop:elitism} assumes exact maximization over the grid, so with sampled sets the monotonicity lives in the bookkeeping --- evaluate $\targetm{\theta}{\cdot}$ at each candidate and retain the best --- and not in the message passing itself.
...
\begin{table*}[h]
    ...
    \begin{tabular}{@{}p{4.1cm}p{3.7cm}p{5.1cm}@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{What each regime costs and what it buys. Elitism (Section~\ref{sec:algorithms}) makes the incumbent nondecreasing in every row at fixed $\theta$, under no assumption on $\gamma_\theta$ beyond measurability and finiteness; Section~\ref{sec:learning} treats the case where $\theta$ moves.}
...
\textbf{Countable support below $T = 1$} (Section~\ref{sec:countable}) has no exact method. The generating-function route holds at $T = 1$ alone, so the whole annealing path is outside it; per-site log-concavity is free in an exponential family but is not closed under max-marginalization; and what remains is either convolution-structured couplings, where closure does hold, or a truncation bound. Stated representationally, the target is a finitely parameterized family of functions on $\mathbb{Z}$ closed under pointwise addition and under partial maximization against the model's couplings. The closure theory of M-convex functions is the obvious candidate, but Section~\ref{sec:countable} reports it failing in both directions: our factors are not M$^{\natural}$-concave, and neither is the convolution-structured case whose closure we can prove, so the condition is sufficient and not necessary and cannot be the characterization sought. What the right condition is remains open.

It is worth being clear about what this gap does and does not block. Countable-site representation belongs to the \emph{shared substrate} of Section~\ref{sec:algorithms} --- the candidate sets --- and not to anything that distinguishes the four algorithms from one another. A representation for countable messages is needed whether one runs contraction, loopy max-product, perturbation, or reweighted message passing. The gap is therefore a \emph{precondition} on applying the substrate to models with countable sites, upstream of and orthogonal to the choice among A1--A4, rather than a tiebreaker between them. On models whose sites are all finite or continuous it blocks nothing at all.
...
\textbf{Iteration counts.} No analysis here bounds the number of sweeps to convergence for A2 or A4; the claim that joint updates need fewer sweeps than coordinatewise ones is argued structurally and observed empirically, not proved. This is less a deficiency of the present design than the ordinary situation in optimization, where iteration counts to a stationary point are rarely available a priori and are characterized by benchmarking instead. It is listed here because it means the cost column of Table~\ref{tab:algorithms} is per sweep, so the table supports comparison on assumptions and guarantees but not on wall-clock, which will have to be settled by running the candidates.
